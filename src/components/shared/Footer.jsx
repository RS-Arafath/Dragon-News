
import Image from 'next/image';
import Link from 'next/link';
import { FaFacebook, FaTwitter, FaInstagram, FaYoutube } from 'react-icons/fa';
import footer_Logo from "@/assets/logo.png"
export default function FooterPage() {
  return (
    <footer className="bg-gray-100 text-gray-600 font-sans border-t border-gray-200">
      {/* Main Footer Container using Responsive Grid Columns */}
      <div className="max-w-7xl mx-auto px-4 py-12 md:py-16 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 md:gap-12">
        {/* Column 1: Brand Logo, Info & Social Links */}
        <div className="flex flex-col gap-4">
          <Image
            src={footer_Logo}
            alt="footer_logo"
            className="font-mystery-quest text-2xl md:text-3xl text-gray-900 tracking-wider"
          ></Image>
          <p className="text-sm text-gray-500 leading-relaxed text-justify">
            Your most reliable source for breaking news. We are committed to
            delivering accurate updates on politics, international affairs,
            sports, technology, and entertainment ahead of the curve.
          </p>
          {/* Social Icons with Smooth Transitions on White/Light Gray Base */}
          <div className="flex gap-4 mt-2">
            <Link
              href="#"
              className="h-9 w-9 flex items-center justify-center rounded-full bg-white border border-gray-200 text-gray-500 hover:bg-[#d82352] hover:text-white hover:border-[#d82352] transition-colors duration-300 shadow-xs"
            >
              <FaFacebook size={18} />
            </Link>
            <Link
              href="#"
              className="h-9 w-9 flex items-center justify-center rounded-full bg-white border border-gray-200 text-gray-500 hover:bg-[#d82352] hover:text-white hover:border-[#d82352] transition-colors duration-300 shadow-xs"
            >
              <FaTwitter size={18} />
            </Link>
            <Link
              href="#"
              className="h-9 w-9 flex items-center justify-center rounded-full bg-white border border-gray-200 text-gray-500 hover:bg-[#d82352] hover:text-white hover:border-[#d82352] transition-colors duration-300 shadow-xs"
            >
              <FaInstagram size={18} />
            </Link>
            <Link
              href="#"
              className="h-9 w-9 flex items-center justify-center rounded-full bg-white border border-gray-200 text-gray-500 hover:bg-[#d82352] hover:text-white hover:border-[#d82352] transition-colors duration-300 shadow-xs"
            >
              <FaYoutube size={18} />
            </Link>
          </div>
        </div>

        {/* Column 2: News Categories Links */}
        <div>
          <h3 className="text-gray-900 font-semibold text-base mb-4 tracking-wide uppercase border-b border-gray-200 pb-2">
            Categories
          </h3>
          <ul className="space-y-2.5 text-sm">
            <li>
              <Link href="#" className="hover:text-[#d82352] transition-colors">
                National
              </Link>
            </li>
            <li>
              <Link href="#" className="hover:text-[#d82352] transition-colors">
                International
              </Link>
            </li>
            <li>
              <Link href="#" className="hover:text-[#d82352] transition-colors">
                Sports
              </Link>
            </li>
            <li>
              <Link href="#" className="hover:text-[#d82352] transition-colors">
                Tech & Science
              </Link>
            </li>
            <li>
              <Link href="#" className="hover:text-[#d82352] transition-colors">
                Entertainment
              </Link>
            </li>
          </ul>
        </div>

        {/* Column 3: Important Quick Links */}
        <div>
          <h3 className="text-gray-900 font-semibold text-base mb-4 tracking-wide uppercase border-b border-gray-200 pb-2">
            Quick Links
          </h3>
          <ul className="space-y-2.5 text-sm">
            <li>
              <Link href="#" className="hover:text-[#d82352] transition-colors">
                About Us
              </Link>
            </li>
            <li>
              <Link href="#" className="hover:text-[#d82352] transition-colors">
                Contact Us
              </Link>
            </li>
            <li>
              <Link href="#" className="hover:text-[#d82352] transition-colors">
                Advertise With Us
              </Link>
            </li>
            <li>
              <Link href="#" className="hover:text-[#d82352] transition-colors">
                Privacy Policy
              </Link>
            </li>
            <li>
              <Link href="#" className="hover:text-[#d82352] transition-colors">
                Terms of Service
              </Link>
            </li>
          </ul>
        </div>

        {/* Column 4: Newsletter Subscription Form */}
        <div>
          <h3 className="text-gray-900 font-semibold text-base mb-4 tracking-wide uppercase border-b border-gray-200 pb-2">
            Newsletter
          </h3>
          <p className="text-sm text-gray-500 mb-4 leading-relaxed">
            Subscribe to our daily newsletter to receive the latest updates
            right in your inbox.
          </p>
          <form className="flex flex-col gap-2">
            <input
              type="email"
              placeholder="Enter your email"
              className="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-lg text-sm text-gray-800 focus:outline-none focus:border-[#d82352] transition-colors shadow-xs placeholder-gray-400"
              required
            />
            <button
              type="submit"
              className="w-full py-2.5 bg-[#d82352] hover:bg-[#b01a40] text-white text-sm font-medium rounded-lg transition-colors cursor-pointer shadow-xs"
            >
              Subscribe
            </button>
          </form>
        </div>
      </div>

      {/* Bottom Copyright Section */}
      <div className="bg-gray-100/50 border-t border-gray-200 py-6">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-gray-400">
          <p>© {new Date().getFullYear()} Dragon News. All rights reserved.</p>
          <p>Developed with ❤️ using Next.js & Tailwind</p>
        </div>
      </div>
    </footer>
  );
}
